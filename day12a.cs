...
class Program {
  ...
  public static int traverse(HashSet<Node> visited, Node current) {
    if(current.Name == "end") {
      return 1;
    }
    int result = 0;
    ...
    foreach(Node n in current.getNeighbors()) {
      if(n.isBig || !visited.Contains(n)) {
        result += traverse(new HashSet<Node>(visited), n);
      }
    }
    return result;
  }
  ...
  public static void Main (string[] args) {
    ...
    Console.WriteLine(traverse(new HashSet<Node>(), start));
  }
}
